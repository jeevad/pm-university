...
class ArticleRepository extends BaseRepository {
	...
	private function queryActiveOrderByDate($topicId, $typeId, $orderBy = 'created_at', $sort = 'desc') {
		$query = $this->model->select ( 'id', 'created_at', 'updated_at', 'title', 'slug', 'user_id', 'description', 'sequence' )->where ( 'topic_id', $topicId )->whereNull ( 'articles.deleted_at' );
	...
	private function queryActiveWithUser($topicId = 1) {
		return $this->model->select ( 'id', 'created_at', 'updated_at', 'title', 'slug', 'user_id', 'description', 'sequence', 'url' )->where ( 'topic_id', $topicId )->whereNull ( 'content.deleted_at' )->with ( 'user' );
	}
	...
	public function saveArticle($article, $inputs, $userId = null) {
		...
		$article->source_url = strtolower ( $inputs ['source_url'] );
		$article->title = ucwords ( strtolower ( $inputs ['title'] ) );
		$article->description = ucwords ( $inputs ['description'] );
		$article->author_name = $inputs ['author_name'] ?? '';
		$article->author_description = $inputs ['author_description'] ?? '';
		$article->author_picture_id = null;
		
		$article->slug = $this->generateSlug ( $article, $inputs ['title'] );
		$article->article_type_id = ( int ) $inputs ['article_type_id'];
		if ($userId) {
